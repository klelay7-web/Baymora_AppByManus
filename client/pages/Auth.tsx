...
export default function Auth() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (mode === 'signature' && !email.trim()) { setError('L\'email est requis en mode Signature'); return; }
    setError('');
    setLoading(true);
    try {
      await register({ pseudo, prenom, email: email || undefined, password: password || undefined, mode });
      navigate(returnTo);
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-slate-900 border border-white/10 rounded-2xl p-6 shadow-2xl">
          ...
          {/* Tabs */}
          <div className="flex gap-1 bg-slate-950/60 rounded-xl p-1 mb-6">
            <button
              onClick={() => setTab('register')}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'register' ? 'bg-secondary text-white shadow-sm' : 'text-white/50 hover:text-white'}`}
            >
              Créer mon compte
            </button>
            <button
              onClick={() => setTab('login')}
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'login' ? 'bg-secondary text-white shadow-sm' : 'text-white/50 hover:text-white'}`}
            >
              Se connecter
            </button>
          </div>
          ...
          {tab === 'register' && (
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <div className="text-sm font-semibold">Mode Fantôme</div>
                    <div className="text-xs text-white/40 mt-0.5">Anonymat total</div>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    <div className="text-lg mb-1">✦</div>
                    <div className="text-sm font-semibold">Mode Signature</div>
                    <div className="text-xs text-white/40 mt-0.5">Être reconnu</div>
                  </button>
                </div>
                <p className="text-white/30 text-xs mt-2">
                  {mode === 'fantome'
                    ? 'Vos données ne sont jamais liées à votre identité réelle.'
                    : 'Baymora peut signaler votre arrivée. Les établissements vous connaissent.'}
                </p>
              </div>
              ...
              <div>
                <label className="text-white/60 text-xs font-medium mb-1.5 block">
                  Email {mode === 'signature' ? <span className="text-secondary">*</span> : <span className="text-white/30">(optionnel en mode Fantôme)</span>}
                </label>
                <input
                  type="email"
                  value={email}
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full h-11 bg-secondary hover:bg-secondary/90 text-white font-semibold rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
              ...
              <a
                ...
                className="w-full h-11 border border-white/15 bg-white/5 text-white/80 font-medium rounded-xl flex items-center justify-center gap-3 hover:bg-white/10 hover:border-white/25 transition-all"
              >
              ...
              <button
                type="button"
                disabled
                className="w-full h-11 border border-white/10 bg-white/5 text-white/40 font-medium rounded-xl flex items-center justify-center gap-3 cursor-not-allowed"
