    ...
    try {
      ...
      if (!rows || rows[0].c === 0) {
        ...
        await conn.query(`CREATE TABLE IF NOT EXISTS \`member_defaults\` (
          ...
          \`enviesDefault\` json DEFAULT NULL,
          ...
          \`budgetRepartition\` json DEFAULT NULL,
          \`createdAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
          \`updatedAt\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  ...
  stripe: router({
    ...
    buyCreditPack: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const StripeLib = (await import("stripe")).default;
        const stripe = new StripeLib(process.env.STRIPE_SECRET_KEY || "", { apiVersion: "2024-06-20" as any });
        const { CREDIT_PACKS } = await import("./stripe/products");
        const pack = CREDIT_PACKS[input.packId];
        if (!pack) throw new TRPCError({ code: "BAD_REQUEST", message: "Pack invalide" });
        const session = await stripe.checkout.sessions.create({
          mode: "payment",
          payment_method_types: ["card"],
          customer_email: ctx.user.email ?? undefined,
          ...
          success_url: `${input.origin}/maya?credits=added&pack=${input.packId}`,
