...
export async function updateUserCredits(userId: number, amount: number, type: string, description: string) {
  ...
  return await db.transaction(async (tx) => {
    ...
    await tx.insert(creditTransactions).values({
      userId, amount, type: type as any, description, balanceAfter: newBalance,
    });
    return newBalance;
  });
}
...
export async function createConversation(userId: number, title?: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(conversations).values({ userId, title: title || "Nouvelle conversation" });
...
export async function addMessage(conversationId: number, role: "user" | "assistant" | "system", content: string, metadata?: string, isVoice = false) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.insert(messages).values({ conversationId, role, content, metadata, isVoice });
...
export async function addCompanion(userId: number, data: { name: string; relationship?: string; dietaryRestrictions?: string; preferences?: string; birthDate?: string }) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function trackAffiliateClick(partnerId: number, clickedUrl: string, seoCardId?: number, userId?: number, referrer?: string, userAgent?: string, ipHash?: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(affiliateClicks).values({ partnerId, seoCardId, userId, clickedUrl, referrer, userAgent, ipHash });
  return result[0].insertId;
}
...
export async function getAffiliateStats() {
  ...
  return {
    ...
    totalConversions: convs?.count || 0,
    totalCommission: parseFloat(convs?.total || "0"),
    pendingCommission: parseFloat(convs?.pending || "0"),
  };
}
...
export async function getAdminStats() {
  const db = await getDb();
  if (!db) return { totalUsers: 0, premiumUsers: 0, totalCards: 0, publishedCards: 0, totalTasks: 0 };
  ...
  return {
    ...
    publishedCards: cardStats?.published || 0,
